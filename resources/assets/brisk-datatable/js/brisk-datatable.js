(function ($) {
        ...
        var internalFunctions = {
            get : function() {
                ...
                .done(function(response){
                    ...
                    $(dataSet).each(function(key, columns){
                        ...
                        rows += '            <input class="custom-control-input checkbox-bulk-select-target" type="checkbox" id="checkbox-' + key + '" />';
                        rows += '            <label class="custom-control-label" for="checkbox-' + key + '"></label>';
                        rows += '        </div>';
                        rows += '    </td>';
            ...
            setTitles: function(response) {
                ...
                if($.trim(internalSettings.datatable.title) == ""){
                    ...
                    $(this).find('[brisk-datatable-title]').html(response.table_title);
                }
            },
                ...
                $.each(columns, function(column_key, column){
                    ...
                    if(column.formatter) {
                        if(typeof internalSettings.formatters[column.formatter] == "function"){
                            value = internalSettings.formatters[column.formatter].call(this, record, column.column, value);
                        }
                        ...
                        if(internalSettings.formatters[column.formatter] !== undefined){
                            if(typeof internalSettings.formatters[column.formatter].render == "function"){
                                value = internalSettings.formatters[column.formatter].render.call(this, record, column.column, value);
                            } 
    ...
    $.fn.briskDataTable.nested_columns_access = function(column, record){
        ...
        //    for(var counter = 0; counter < columns.length - 1; counter++){
        //        console.log(counter);
        //    }

            if(columns.length == 2){ //to do
                
            }

            if(columns.length == 3){
                var value = [];
                var first_level = record[columns[0]];

                if($.isArray(first_level)){
                    $(first_level).each(function(){
                        if(this[columns[1]] && this[columns[1]][columns[2]]){
                            value.push(this[columns[1]][columns[2]]);
                        }
                    });
                }
            ...
            if(columns.length == 4){
                var value = [];
                var first_level = record[columns[0]][columns[1]];

                if($.isArray(first_level)){
                    $(first_level).each(function(){
                        if(this[columns[2]] && this[columns[2]][columns[3]]){
                            value.push(this[columns[2]][columns[3]]);
                        }
                    });
                }
